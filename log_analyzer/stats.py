...
class StatsSummary:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "server error_count": self.server_error_count,
            "client_error_rate_percent": round(self.client_error_rate, 2),
            "server_error_rate_percent": round(self.server_error_rate, 2),
...
class StatsAggregator:
    ...
    def summary(self, top_n: int = 10) -> StatsSummary:
        ...
        client_error_rate=(self._client_error_count / total * 100) if total else 0.0
        server_error_rate=(self._server_error_count / total * 100) if total else 0.0
        top_endpoints = self._endpoint_counts.most_common(top_n)

        hourly_distribution = {}
        for hour in range(24):
            count = self._hourly_counts.get(hour, 0)
